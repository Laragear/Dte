...
Since a DTE with a business error will keep failing if automatically retried, you must resolve the data yourself. You can catch the event, correct the data on your end, and try again using `$dte->replicateForRetry()`, which safely clones the payload and readies the document to acquire a fresh folio.
...
If a folio was skipped, damaged, or cannot be used, you must report it to the SII to keep your records aligned. You can use the `FolioAnnulmentService` to report these folios directly to the SII:

```php
use Laragear\Dte\Services\FolioAnnulmentService;
use Laragear\Dte\Models\SiiDte;

$service = app(FolioAnnulmentService::class);

// Report a damaged or annulled DTE folio
$service->annul(SiiDte::find(1));
```
...
5. **Compliance:** The SII will let you digitally sign your compliance to operate on production.
...
> [!WARNING]
>
> Once you sign, you will no longer be able to access the SII Web App to manage DTE. Backup all your historical data before. If you consider this a drawback, desist and use this library as manual mirror.
