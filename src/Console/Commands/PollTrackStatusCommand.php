...
class PollTrackStatusCommand extends Command
{
    ...
    {
        // Don't poll envelopes that were just uploaded or just polled recently.
        // We use the same holding minutes config as a reasonable interval (e.g. 30 mins)
        ...
        /** @var LazyCollection<int, SiiDteEnvelope> $envelopes */
        $envelopes = SiiDteEnvelope::query()
        ...
        foreach ($envelopes as $envelope) {
            ...
            // We advance the updated_at timestamp by the delay. This ensures that
            // if there's a massive queue of polling jobs (e.g., 500 jobs * 60s = 8.3 hours),
            // the cron won't pick up this envelope again until 30 minutes *after*
            // the job is actually scheduled to run.
