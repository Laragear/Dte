...
class CheckNearDepletedCafsCommand extends Command
{
     ...
     */
    public function handle(Dispatcher $event, Repository $config): int
    {
        ...
        SiiCaf::query()
            ...
            ->chunk(100, function ($cafs) use ($threshold, $event) {
                foreach ($cafs as $caf) {
                    ...
                    if ($caf->expires_on) {
                        $daysLeft = (int) now()->startOfDay()->diffInDays($caf->expires_on->startOfDay(), false);
