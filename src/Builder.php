...
class Builder
{
    ...
    /**
     * Create a hydrated builder to retry a rejected or failed document.
     */
    public function retry(SiiDte $dte): DocumentBuilder
    {
        $builder = match ($dte->document_type) {
            DteType::Invoice, DteType::InvoiceExempt => $this->app->make(InvoiceBuilder::class),
            DteType::Receipt => $this->app->make(ReceiptBuilder::class),
            DteType::CreditNote => $this->app->make(CreditNoteBuilder::class),
            DteType::DebitNote => $this->app->make(DebitNoteBuilder::class),
            DteType::DispatchGuide => $this->app->make(DispatchGuideBuilder::class),
            DteType::PurchaseInvoice => $this->app->make(PurchaseInvoiceBuilder::class),
            DteType::InvoiceLiquidation => $this->app->make(InvoiceLiquidationBuilder::class),
            default => throw new LogicException(
                "DTE type [{$dte->document_type->value}] does not support retry.",
            ),
        };

        return $builder->hydrate($dte);
    }
}
